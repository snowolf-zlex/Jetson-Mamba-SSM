...
# version of PyTorch. The following code is for backward compatibility with
# older PyTorch and for PyTorch builds that don't have these APIs (e.g., Jetson).
def _ensure_distributed_compatibility():
    """Ensure torch.distributed has the required APIs."""
    if "all_gather_into_tensor" not in dir(torch.distributed):
        if hasattr(torch.distributed, '_all_gather_base'):
            torch.distributed.all_gather_into_tensor = torch.distributed._all_gather_base
        else:
            # Create a dummy function that raises an error when actually used
...
_ensure_distributed_compatibility()
